    ...
    */
    ...
    public function authenticate(Requests\AuthRequest $request)
    {
        ...
        if (Auth::attempt(['name' => $data['name'], 'password' => $data['password']])) 
        {
            ...
            {
                Auth::logout();
                return redirect('/')->withInput($request->only('user_status_id'))->withErrors(['name' => 'Sua conta encontra-se AGUARDANDO sua confirmacao ! Favor verificar seus e-mails.']);
            }
            ...
            {
                Auth::logout();
                return redirect('/')->withInput($request->only('user_status_id'))->withErrors(['name' => 'Sua conta não está DESATIVADA !']);
            }
        }
        else
        {
            return redirect('/')->withInput($request->only('user_status_id'))->withErrors(['name' => 'Não foi possível confirmar suas credenciais !']); 
        }
    }
